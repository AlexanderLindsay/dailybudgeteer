...
export class WeeklyGraphController {
...
export class WeeklyGraphComponent implements
    m.ClassComponent<WeeklyGraphController> {

    private loadGraphData: (data: Expense[]) => void;

    constructor(private context: BudgetContext) { }

    public oninit = (node: m.CVnode<WeeklyGraphController>) => {
        let date = m.route.param("date");
        let day = moment(date);
        node.attrs.day(day);
    }

    public view = (node: m.CVnode<WeeklyGraphController>) => {
        let ctrl = node.attrs;
        return m("div", { id: "graph" });
    }

    public onupdate = (node: m.CVnode<WeeklyGraphController>) => {
        this.loadGraphData(node.attrs.expenses());
    }

    public oncreate = (node: m.CVnode<WeeklyGraphController>) => {
